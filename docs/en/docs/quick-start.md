...
First, you need to create a `Database` object, which is responsible for connecting to and interacting with the database. You need to pass in a database connection string, for example:
...
You can define a database Model just as easily as `Pydantic` by inheriting the `cherry.Model` class.
...
The data model requires a primary key field，such as `id`，declare it as a primary key using `Field(primary_key=True)`。

You also need to specify the database object and table name in the `Meta` class of the data model.(1)
{ .annotate }

1. You can leave the table name unspecified, which defaults to using the model's class name.


## Create table

You need to call `db.create_all()` at the entrance of your main program to create all the tables.
...
Here are some simple examples to demonstrate the addition, deletion, modification and checking of the data model.
...
??? note "Examples involving table association and more complex query methods"
